...
export function patchTurboJson(
  cwd: string,
  config: FwizConfig,
): WorkspacePatchResult {
  const filePath = join(cwd, 'turbo.json');

  if (!existsSync(filePath)) {
    return { patched: false, filePath, changes: [] };
  }

  let turboJson: Record<string, unknown>;

  try {
    turboJson = JSON.parse(readFileSync(filePath, 'utf8')) as Record<
      string,
      unknown
    >;
  } catch {
  ...
  const existing = turboJson.fwiz;

  if (JSON.stringify(existing) === JSON.stringify(fwizSection)) {
    return { patched: false, filePath, changes: [] };
  }

  turboJson.fwiz = fwizSection;
  writeFileSync(filePath, `${JSON.stringify(turboJson, null, 2)}\n`, 'utf8');

  return {
    patched: true,
    filePath,
    changes: ['Updated turbo.json fwiz project references'],
  };
}
